...
say "# -> 800KB file (200 blocks)";
$chunks = 800 * 1024 / 16;
$content = "0123456789ABCDEF" x $chunks;
write_text("huge.txt", $content);
$size = -s "mnt/huge.txt";
$size or $size = 0;
ok($size eq 16 * $chunks + 1, "800KB file has correct size");
$back = read_text("huge.txt");
ok($content eq $back, "800KB file content verified");
...
for my $i (1..5) {
    $chunks = 100 * 1024 / 16;
    my $data = sprintf("%016d", $i) x $chunks;
    push @contents, $data;
    write_text("big$i.txt", $data);
}
my $all_ok = 1;
for my $i (1..5) {
    my $sz = -s "mnt/big$i.txt";
    $sz or $sz = 0;
    $all_ok = 0 unless $sz eq 100 * 1024 + 1;
    my $rd = read_text("big$i.txt");
    $all_ok = 0 unless $rd eq $contents[$i-1];
}
ok($all_ok, "5 x 100KB files created and verified");
